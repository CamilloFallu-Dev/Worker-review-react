...
import { useGetCompanyBySlugQuery, useGetReviewsQuery } from "../services/apiService";
...
export default function PaginaAzienda() {
  ...
  const { data: reviewsData, error: reviewsError, isLoading: reviewsLoading } = useGetReviewsQuery(azienda?.id, {
    skip: !azienda,
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!azienda || nome.trim() === "" || titolo.trim() === "" || recensione.trim() === "") return;
    ...
    <div>
      <div className="p-4 bg-gray-50 min-h-screen flex flex-col items-center justify-center">
        ...
        <div className="w-full p-6 flex flex-col items-center mt-10">
          ...
          <p className="mt-2 text-gray-600 w-3xl flex justify-center">{azienda.description}</p>
